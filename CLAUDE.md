...
**Published Artifact URL**: https://claude.ai/code/artifact/69c74289-17b2-4a5e-af17-0fa02a4177d4 — pass this as `url` when republishing an updated build so it updates in place instead of creating a new artifact. Kept in sync with `main` as of 2026-09-02 (includes the left-sidebar issuer navigation, see "UI direction" below); republish after any further UI change if the user is actively sharing this link. Update this line if the user shares a different/newer URL.

## Deployed as a real website (Vercel + GitHub)

As of 2026-09-02, the app is also deployed as a proper website, mirroring how the sibling `Pokemon Project` is deployed (`pokepedia-ten.vercel.app`, GitHub-connected Vercel project) — a second, independent distribution channel alongside the single-file Artifact above.

- **GitHub repo**: https://github.com/prabhanjanbalakrishnan/Card-Compare.git (remote `origin`, branch `main`). Created empty by the user, then this local repo was pushed to it directly (`git push -u origin main`) — no separate build/export step, since this is a normal Vite source push, not the inlined single-file artifact.
- **Live URL**: https://card-compare-ashy.vercel.app
- **Vercel project settings** (set once via the dashboard at import time, persist automatically): Framework = Vite (auto-detected), **Root Directory = `app`** (important — the Vite project lives in `app/`, not the repo root, so this must be set explicitly during import; Vercel's default build/output settings for Vite, `npm run build` / `dist`, are correct once the root directory is right and didn't need overriding). No environment variables needed (no backend/API keys — see project overview).
- **Auto-deploy**: the Vercel project is connected to the GitHub repo, so every `git push` to `main` triggers a new production deployment automatically — **no manual redeploy step**, unlike the Artifact link above which requires an explicit rebuild-and-republish each time. This makes the Vercel URL the lower-friction one to keep current; consider it the primary link to share going forward, with the Artifact link as a secondary/backup.
- **Setup gotcha**: the Vercel GitHub App's repo access is opt-in per-repo (not "all repos" by default) — after creating a new GitHub repo, Vercel's import picker won't show it until you visit "Adjust GitHub App Permissions" (from the same-titled link on the Vercel new-project page) and explicitly grant it access to the new repo, then return to Vercel and reload the import page.
- Routing works with no server-side rewrites needed since the app already uses `HashRouter` (see project overview) — confirmed by loading `/#/amex` directly on the deployed domain.
...
- **Per-card `flags` render as a neutral disclaimer, not a warning** (changed 2026-09-21): `CardDetail.jsx`'s flags section was renamed `.flags-box` → `.disclaimer-box` and restyled off the amber `--warn`/`--warn-soft` tokens onto the same neutral `--accent-soft`/`--rule` look used elsewhere for notices — a spot-check list next to a "Buy this card" context read as a scary warning label, not what the amber styling was meant to convey. It now closes with a link to that card's issuer `website` (looked up from `ISSUERS` by `card.issuer` name, same URL as the `IssuerPage` link) reading "For the most accurate, up-to-date information, visit {Issuer}'s official card page ↗" — pointing users at the source instead of just flagging uncertainty with no next step. `--warn`/`--warn-soft` are still used elsewhere (the Find My Card over-budget note), so they weren't removed from `index.css`.
